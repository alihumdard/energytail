...
export default function RolesPermissionsPage() {
  ...
  /** Which role dialog is open: creating a new one, or renaming an existing. */
  ...
  const activeRole: Role | undefined = roles.find(
    (r) => r.id === effectiveRoleId,
  );
  ...
  return (
    <AdminShell active="roles">
      ...
      <p className="text-sm text-slate-500 -mt-4">
        Manage user roles and their permissions. Control what each role can
        access and modify across the platform.
      </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-6 items-start">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-4">
              ...
              return (
                ...
                >
                  ...
                  >
                    {r.is_system
                      ? "System"
                      : (r.users_count ?? 0).toLocaleString()}
                  </span>
                </button>
              );
            })}
          ...
          <button
            onClick={() => setDialog({ mode: "create" })}
            className="w-full flex items-center justify-center gap-2 mt-3 border-2 border-dashed border-slate-200 rounded-xl py-3 text-sm font-medium text-slate-500 hover:bg-slate-50"
          >
            <Plus className="w-4 h-4" />
            Add New Role
          </button>
          ...
          <div className="p-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-1">
              <div>
                ...
                <p className="text-sm text-slate-400 mt-0.5">
                  Define what actions the {activeRole?.label ?? "selected"} role
                  can perform.
                </p>
              </div>
              ...
              {activeTab === "Permissions" && (
                <div className="flex items-center gap-2.5 shrink-0">
                  ...
                  {/* Renaming is separate from the grid: the name and
                          description are role metadata, the checkboxes are
                          the permissions, and they save independently. */}
                  {activeRole && (
                    <button
                      onClick={() =>
                        setDialog({ mode: "edit", role: activeRole })
                      }
            ...
            {locked && activeTab === "Permissions" && (
              <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                The Administrator role always holds every permission and cannot
                be edited — otherwise the platform could be left with nobody
                      ...
                      {modules.map((m) => (
                        ...
                        >
                          ...
                          {actions.map((a) => {
                            ...
                            const checked =
                              locked || selected.has(cell.permission);
                            ...
                            return (
                              <td key={a.key} className="text-center">
                                <button
                                  ...
                                  /*
                                   * A locked cell is drawn grey rather than in
                                   * the accent colour. Dimming a blue tick by a
                                   * little still read as an ordinary toggle, so
                                   * clicking it and seeing nothing happen looked
                                   * like a broken checkbox rather than a rule.
                                   */
      ...
      {dialog && (
        <RoleFormModal
          ...
          onSaved={(role) => {
            refetchRoles();
            // Select a newly created role so its empty grid is right there to
            // fill in — creating one and then hunting for it is busywork.

...
export default function AuditLogsPage() {
  ...
  const { data: filtersResponse } = useApiResource(() => adminAudit.filters(), []);
  ...
  const stats = statsResponse?.data as
    | { total: number; today: number; failed: number; failed_logins_week: number }
    | undefined;
        ...
        <main className="flex-1 p-4 sm:p-6 space-y-6">
          ...
          {stats && (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { label: "Total Events", value: stats.total, tone: "text-slate-900" },
                { label: "Today", value: stats.today, tone: "text-slate-900" },
                { label: "Failed Events", value: stats.failed, tone: "text-red-600" },
                {
                  label: "Failed Logins (7d)",
                  value: stats.failed_logins_week,
                  // The number worth watching for a brute-force attempt.
                  tone: stats.failed_logins_week > 0 ? "text-orange-600" : "text-slate-900",
                },
              ].map((card) => (
          ...
          <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
            ...
            <div className="flex flex-wrap items-center gap-3 p-4 border-b border-slate-100">
              ...
              {[
                { value: module, set: setModule, options: filters?.modules, label: "All Modules" },
                { value: action, set: setAction, options: filters?.actions, label: "All Actions" },
                { value: role, set: setRole, options: filters?.roles, label: "All Roles" },
                { value: status, set: setStatus, options: filters?.statuses, label: "All Statuses" },
              ].map((f) => (
                <select
                  key={f.label}
                  value={f.value}
                    ...
                    entries.map((log) => (
                      ...
                      >
                        ...
                        <td className="px-2 py-3.5">
                          <div className="text-slate-700">{log.user ?? "System"}</div>
                          <div className="text-xs text-slate-400">{humanise(log.role)}</div>
                        </td>
                        <td className="px-2 py-3.5">
                          <span
                            className={`text-xs font-medium px-2 py-1 rounded whitespace-nowrap ${
                              ACTION_STYLE[log.action ?? ""] ?? "bg-slate-100 text-slate-600"
                            }`}
                          >
                            {humanise(log.action)}
                          </span>
                        </td>
                        <td className="px-2 py-3.5 text-slate-500">{humanise(log.module)}</td>
                        <td className="px-2 py-3.5 text-slate-600 max-w-xs truncate" title={log.description}>
                          {log.description}
                        </td>

...
import MailHealthPanel from "@/components/admin/MailHealthPanel";
...
export default function SettingsPage() {
  const {
    ...
    error,
    refetch,
  } = useApiResource(() => adminSettings.list(), []);
  ...
  async function save() {
    const changed = Object.entries(edits).map(([key, value]) => ({
      key,
      value,
    }));
      ...
      {!loading && !error && (
        <div className="grid grid-cols-1 lg:grid-cols-[220px_1fr] gap-6 items-start">
          ...
          <div className="space-y-6">
            {/*
                  Only under Email: it answers the question that group raises —
                  are these settings actually delivering anything?
                */}
            {activeGroup === "email" && <MailHealthPanel />}
            ...
            <div className="bg-white rounded-2xl border border-slate-100 p-5 space-y-5">
              ...
              {items.map((item) => {
                ...
                return (
                  <div
                    key={item.key}
                    className="grid grid-cols-1 sm:grid-cols-[220px_1fr] gap-3 items-start"
                  >
                    <div>
                      <label
                        htmlFor={item.key}
                    ...
                    {item.type === "integer" && (
                      <input
                        ...
                        value={String(value ?? "")}
                        onChange={(e) =>
                          setValue(item.key, Number(e.target.value))
                        }
                    ...
                    {!["boolean", "file", "integer", "json"].includes(
                      item.type,
                    ) && (
                      <input
                        id={item.key}
